...
namespace SkierFramework
{
    ...
    public class UIManager : SingletonMono<UIManager>
    {
        ...
        {
            // 初始化需要加载所有UI的配置
            var list = await UIConfig.GetAllConfigsAsync();
            if (list == null) return;
        ...
        public UniTask Preload(UIType type)
        {
        ...
        {
            if (_fadeTween.isAlive) _fadeTween.Complete();
            _fadeTween = Tween.Alpha(_blackMask, 1.0f, duration);
            if (callback != null)
            {
        ...
        {
            if (_fadeTween.isAlive) _fadeTween.Complete();
            _fadeTween = Tween.Alpha(_blackMask, 0.0f, duration);
            if (callback != null)
            {
                _fadeTween.OnComplete(callback);
            }
        ...
        public void FadeInOut(float duration = 1.0f, Action callback = null)
        {
            if (_fadeTween.isAlive) _fadeTween.Complete();
            // 先淡入到 1，再淡出回 0
            var sequence = Sequence.Create()
                .Chain(Tween.Alpha(_blackMask, 1.0f, duration * 0.5f))
                .Chain(Tween.Alpha(_blackMask, 0.0f, duration * 0.5f));
            if (callback != null)
            {
